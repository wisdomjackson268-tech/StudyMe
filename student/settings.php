...
if (is_post() && isset($_POST['change_password'])) {
    ...
    if (empty($errors)) {
        ...
        log_user_activity($userId, 'password_changed', 'Student changed account password');
        set_flash('success', 'Your password has been changed successfully.');
        redirect('student/settings.php');
    }
}

// 2. Handle Account Disablement / Deactivation
if (is_post() && isset($_POST['disable_account'])) {
    ...
    if (!password_verify($confirmPassword, $userRecord['password'])) {
        ...
        try {
            ...
            log_user_activity($userId, 'account_disabled', "Student disabled account. Reason: {$disableReason}");
            ...
            session_start();
            set_flash('info', 'Your account has been deactivated. You can contact support anytime if you wish to reactivate your access.');
            redirect('auth/login.php');
            exit;
        } catch (Exception $e) {
            error_log("Error disabling account: " . $e->getMessage());
...
<div class="d-flex align-items-center justify-content-between flex-wrap gap-3 mb-4">
    <div>
        ...
        <p class="text-muted mb-0 small">Manage password security, platform preferences, UI themes, and account status.</p>
    </div>
</div>
    ...
    <div class="col-12 col-lg-8">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 mb-4 bg-white">
            ...
            <form action="<?= url('student/settings.php') ?>" method="POST">
                ...
                <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold shadow-sm" data-feedback="click">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 mb-4 bg-white">
            ...
            <div class="d-flex align-items-center justify-content-between py-3 border-bottom">
                <div>
                    <h6 class="fw-bold mb-1 text-dark">Theme Appearance</h6>
            ...
            <div class="d-flex align-items-center justify-content-between py-3 border-bottom">
                <div>
                    <h6 class="fw-bold mb-1 text-dark">UI Sound Feedback</h6>
                    <p class="text-muted small mb-0">Enable audio feedback on button clicks and exam timer alerts.</p>
                </div>
                <button type="button" class="btn btn-outline-secondary rounded-pill px-4 sound-btn" data-feedback="click">
                    <i class="bi bi-volume-up me-1"></i> Toggle Sounds
                </button>
            </div>

            <div class="d-flex align-items-center justify-content-between py-3">
                <div>
                    <h6 class="fw-bold mb-1 text-dark">Profile Settings</h6>
                    <p class="text-muted small mb-0">Update your avatar, full name, phone number, and academic track.</p>
                </div>
                <a href="<?= url('student/profile.php') ?>" class="btn btn-outline-primary rounded-pill px-4 fw-semibold">
        ...
        <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 mb-4 bg-white border-top border-4 border-danger">
            ...
            <h5 class="fw-bold text-dark mb-2">Disable / Deactivate Account</h5>
            <p class="text-muted small mb-4 lh-base">
                Temporarily deactivating your account will disable your login access, pause email notifications, and sign you out across all devices. 
                Your course progress, past questions history, certificates, and referral wallet balance will remain securely archived in the database.
            </p>
            ...
            <div>
                <button type="button" class="btn btn-outline-danger rounded-pill px-4 fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#disableAccountModal">
                    <i class="bi bi-person-x-fill me-1"></i> Disable My Account
                </button>
            </div>
        </div>
        ...
        <div class="modal-content rounded-4 border-0 shadow">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold text-danger">
                    <i class="bi bi-exclamation-triangle-fill me-2"></i>Deactivate Your Account
                </h5>
            ...
            <form action="<?= url('student/settings.php') ?>" method="POST">
                ...
                <div class="modal-body py-4">
                    <div class="alert alert-warning rounded-3 small p-3 mb-3">
                        ...
                        Are you sure you want to deactivate your StudyMe student account? You will be logged out immediately.
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="disable_reason" class="form-select rounded-3">
                            <option value="Taking an academic break">Taking a temporary study break</option>
                            <option value="Finished WAEC/JAMB Exams">I have finished my WAEC / JAMB exams</option>
                            <option value="Switching to another track">Switching academic track</option>
                            <option value="Privacy / Personal reasons">Personal / Privacy reasons</option>
                            <option value="Other">Other</option>
                        </select>
                    </div>

                    <div class="mb-3">
                        <label for="confirmPasswordDisable" class="form-label small fw-bold text-dark">
                            Confirm Your Password <span class="text-danger">*</span>
                        </label>
                        <input type="password" name="confirm_password_disable" id="confirmPasswordDisable" class="form-control rounded-3" placeholder="Enter your current password" required>
                    </div>

                    <div class="form-check small text-muted">
                        <input class="form-check-input" type="checkbox" id="confirmCheckbox" required>
                        <label class="form-check-label" for="confirmCheckbox">
                            I understand that my account will become inactive until I request reactivation.
                        </label>
                    </div>
                </div>
