...
log_user_activity($userId, 'accessed_waec_hub', 'Student accessed WAEC SSCE Preparation Portal');

// Fetch WAEC Subjects with real question counts
$waecSubjects = get_secondary_exam_subjects('waec');
$allSubjects = get_secondary_subjects('active');

// If no questions specific to WAEC yet for some subjects, supplement with all subjects
$subjectMap = [];
foreach ($waecSubjects as $ws) {
    $subjectMap[$ws['subject_slug']] = $ws;
}
...
// Fetch past WAEC attempts for student
$waecAttempts = [];
if ($studentId) {
    try {
        $stmtAtt = $pdo->prepare("
            ...
            WHERE a.student_id = ? AND a.exam_type = 'waec'
            ORDER BY a.started_at DESC
            LIMIT 6
        ");
        $stmtAtt->execute([$studentId]);
        $waecAttempts = $stmtAtt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {}
}

$pageTitle = 'WAEC SSCE Past Questions & CBT Hub | StudyMe';
    ...
    <div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-1">
                ...
                <li class="breadcrumb-item active" aria-current="page">WAEC SSCE</li>
            </ol>
        </nav>
        <h2 class="fw-bold mb-0 text-dark">
            <i class="bi bi-award-fill text-warning me-2"></i>WAEC SSCE Exam Simulator &amp; Past Questions
        </h2>
        <p class="text-muted mb-0 small">Master West African Senior School Certificate Examination past papers with step-by-step verified solutions.</p>
    </div>

    <div class="d-flex gap-2">
        <a href="<?= url('student/secondary-past-questions.php?exam=waec') ?>" class="btn btn-outline-primary rounded-pill px-3 fw-semibold">
            <i class="bi bi-search me-1"></i> Browse All WAEC Questions
        </a>
    </div>
</div>

<!-- WAEC HERO BANNER -->
<div class="card border-0 rounded-4 shadow-sm overflow-hidden mb-4" style="background: linear-gradient(135deg, #1e3a8a 0%, #0f172a 100%);">
    <div class="card-body p-4 p-md-5 text-white position-relative">
        <div class="row align-items-center">
            <div class="col-lg-8">
                <span class="badge bg-warning text-dark rounded-pill px-3 py-1 mb-2 fw-bold text-uppercase">
                    Official WAEC SSCE Syllabus Aligned
                </span>
                <h3 class="fw-bold text-white mb-2">Simulate Real WAEC SSCE Examination Papers</h3>
                <p class="text-light opacity-75 mb-4">
                    Choose your subject and year to browse verified questions with step-by-step solutions or launch a timed mock exam.
                </p>
                ...
                <form method="GET" action="<?= url('student/secondary-practice.php') ?>" class="row g-2 bg-white bg-opacity-10 p-3 rounded-4 backdrop-blur">
                    <input type="hidden" name="exam" value="waec">
                    ...
                    <div class="col-md-3">
                        <select name="count" class="form-select rounded-3 bg-white border-0">
                            ...
                            <option value="50">50 Questions (Full WAEC)</option>
                            <option value="all">All Available</option>
                        </select>
                    </div>

                    <div class="col-md-2">
                        <button type="submit" class="btn btn-warning text-dark fw-bold rounded-3 w-100">
            ...
            <div class="col-lg-4 text-center d-none d-lg-block">
                <i class="bi bi-award text-warning" style="font-size: 8rem; opacity: 0.85;"></i>
            </div>
        </div>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-header bg-white p-3 p-md-4 border-bottom d-flex align-items-center justify-content-between">
        <h5 class="fw-bold mb-0 text-dark">
            <i class="bi bi-grid-fill text-primary me-2"></i>Select Subject to Browse or Practice
        </h5>
                ...
                <div class="col-md-6 col-lg-4">
                    <div class="card border rounded-4 h-100 p-3 bg-light bg-opacity-25 transition-hover">
                        ...
                        <div class="d-flex gap-2 mt-auto pt-2 border-top">
                            <a href="<?= url('student/secondary-past-questions.php?exam=waec&subject=' . urlencode($s['subject_slug'])) ?>" class="btn btn-sm btn-outline-secondary rounded-pill flex-grow-1">
                                <i class="bi bi-eye me-1"></i> Read Questions
                            </a>
                            <a href="<?= url('student/secondary-practice.php?exam=waec&subject=' . urlencode($s['subject_slug']) . '&count=20') ?>" class="btn btn-sm btn-warning text-dark fw-bold rounded-pill px-3">
...
<!-- RECENT WAEC ATTEMPTS -->
<?php if (!empty($waecAttempts)): ?>
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-header bg-white p-3 p-md-4 border-bottom d-flex align-items-center justify-content-between">
        <h5 class="fw-bold mb-0 text-dark">
            <i class="bi bi-clock-history text-primary me-2"></i>Recent WAEC Mock Attempts
        </h5>
        <a href="<?= url('student/secondary-practice-history.php') ?>" class="btn btn-sm btn-outline-primary rounded-pill px-3">View Full History</a>
    </div>
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            <tbody>
                <?php foreach ($waecAttempts as $att): ?>
                <tr>
                    ...
                    <td class="text-end pe-4">
                        <a href="<?= url('student/secondary-practice.php?review_attempt=' . (int)$att['id']) ?>" class="btn btn-sm btn-outline-primary rounded-pill px-3">
